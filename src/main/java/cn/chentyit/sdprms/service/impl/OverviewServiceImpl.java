...
public class OverviewServiceImpl implements OverviewService {
    ...
    public Map<String, Integer> getSummaryData() {
        ...
        // 获取学者数量
        scholarLambdaQuery.isNotNull(Scholar::getInstituteId).isNotNull(Scholar::getScholarName);
        result.put("scholar_num", scholarMapper.selectCount(scholarLambdaQuery));

        // 获取主题数量
        themeLambdaQuery.isNotNull(Theme::getThemeId).isNotNull(Theme::getThemeName);
        result.put("theme_num", themeMapper.selectCount(themeLambdaQuery));

        // 获取论文数量
        thesisLambdaQuery.isNotNull(Thesis::getThemeId)
                .isNotNull(Thesis::getThesisTitle)
